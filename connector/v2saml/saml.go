...
func (cfg *Config) Open(logger log.FieldLogger) (SAMLConnector, error) {
	return cfg.openConnector(logger)
}

//openConnector ... redundant method
func (cfg *Config) openConnector(logger log.FieldLogger) (interface {
	SAMLConnector
}, error) {
	fmt.Println("Config::openConnector")
	...
	p := &provider{
		...
		groupsDelim:  cfg.GroupsDelim,
		redirectURI:  cfg.RedirectURI,
		logger:       logger,

		nameIDPolicyFormat: cfg.NameIDPolicyFormat,
	}
	...
	if !cfg.InsecureSkipSignatureValidation {
		if (cfg.CA == "") == (cfg.CAData == nil) {
			return nil, errors.New("must provide either 'ca' or 'caData'")
		}

		var caData []byte
		if cfg.CA != "" {
			data, err := ioutil.ReadFile(cfg.CA)
			if err != nil {
...
// Function to build SAML authentication request XML.
// This does not perform the actual post.
func (p *provider) POSTData(s Scopes, id string) (action, value string, err error) {
...
func (p *provider) HandlePOST(s Scopes, samlResponse, inResponseTo string) (ident Identity, err error) {
	ident = Identity{}
	...
	//[adt] p.issuer is in fact the SP (dex) (as set in the AuthnRequest)
	//      resp.Issuer.Issuer is in fact the IDP against which we authenticate, so they won't ever match!
	//      So, if p.issuer is non-empty, it should match response audience field
	...
	ii := 0
	for range attributes.Attributes {
		ii++
	}
	...
	// [adt]
	// oidc.Identity used by dex v1 does not have concept of groups.
	// Leave for now.
	...
	// 	}
	// }
	log.Warn("TODO:(adtrsa) Group attributes ignored.")
	return ident, nil
}
